...
_左侧为演示表单与进度：**选择文件 → 开始上传**，分片阶段可 **暂停 / 继续**，展示 **MD5 计算耗时** 与 **自开始到结束的总耗时**，完成后给出成品链接；刷新后再次选择 **同名、同大小、同 lastModified** 的文件可依赖 **`localStorage` 中的 `uploadId`** 自动 **断点续传**。右侧开发者工具可见其调用 **`POST /api/uploads/large/init`**、**`GET …/status`** 与按索引上传分片等与 OpenAPI 一致的链路。另有类目 Feed **UI 演示**路由 **`/demo/category-feed`**（**无需登录**，当前使用本地 demo 数据，blankLayout；接入后端时可在 composable 中替换数据源）。_
...
| `AUTH_COOKIE_SECURE`                                     | 可选；刷新令牌 Cookie 是否要求 HTTPS：`1`/`true` 恒开、`0`/`false` 恒关、**`auto`** 按本次请求是否 HTTPS 判定（需配 `TRUST_PROXY` 才能识别 `X-Forwarded-Proto`）。生产默认开启；本地 http 调试默认关闭。**若生产入口只有明文 HTTP，恒开会让浏览器静默丢弃该 Cookie（表现为「能登录、刷新页面即掉登录态」），此时应在网关终止 TLS 或改用 `auto`** |
| `REFRESH_ROTATION_GRACE_SECONDS`                         | 可选；刷新令牌轮换后的并发宽限秒数，默认 **30**。多标签页/门户与管理端共用同一枚刷新 Cookie，窗口内的重复提交按**幂等重放**处理——原样返回当初那一枚继任令牌，既不判重放也不补发新的；设 **`0`** 为严格模式                                                                                                                                       |
| `REFRESH_SESSION_ABSOLUTE_TTL_SECONDS`                   | 可选；**绝对会话寿命**（秒），默认 **2592000（30 天）**，`0` 关闭。滑动过期单独用是有洞的：每轮换一次就重置一次有效期，只要保持每周刷新就能无限期续下去。这条硬线让每次登录到点必须重新输密码                                                                                                                                                    |
| `API_DOCS_ENABLED`                                       | 可选；是否挂载 `/api-docs` 与 `/openapi.yaml`。**未设置时生产关闭、其余环境开启**。openapi.yaml 是最完整的一份攻击面清单，生产需要时显式设 `1`，并让入口只经内网/鉴权后可达                                                                                                                                                                      |
| `RATE_LIMIT_GLOBAL_MAX` 等 `RATE_LIMIT_*`                | 可选；限流阈值，默认全局 15 分钟 / 1000 次、注册登录 1 分钟 / 10 次、刷新接口 1 分钟 / 30 次（只统计**携带了刷新 Cookie** 的失败请求；没带凭证的尝试不占额度，否则匿名访客的首屏会把桶吃满）。**计数存于 Redis（key 前缀 `rl:`），多副本共用同一个桶**；Redis 异常时放行并记 error 日志，不因限流把请求打成 500                                  |
| `SHUTDOWN_TIMEOUT_MS`                                    | 可选；收到 SIGTERM 后等待在途请求收尾的上限，默认 **8000**；须小于编排层宽限期（`docker stop` 默认 10s）                                                                                                                                                                                                                                         |
| `SHUTDOWN_DRAIN_MS`                                      | 可选；**摘流量等待**（毫秒），默认 **0**。收到 SIGTERM 后 `/ready` 立刻返回 503，但探针是轮询的——翻完就关监听会让编排层来不及摘除本实例，滚动更新期间出现零星 502。该值即留给编排层「看见并摘掉」的时间，取略大于一个就绪探针周期。单机 Compose 无需，K8s/多副本应显式配置                                                                       |
...
**pc-portal / pc-admin 前端环境**（各 app 目录下的 `.env.development`，示例见 [`apps/frontend/pc-portal/.env.example`](apps/frontend/pc-portal/.env.example)）：

| 变量                       | 说明                                                                                                                                 |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `VITE_API_BASE_URL`        | 留空时使用相对路径 `/api`、`/uploads`（经 Vite proxy 或 Docker 网关同源）；宿主直连后端时可设为 `http://127.0.0.1:3000`              |
| `VITE_DEV_PROXY_TARGET`    | Vite dev 将 `/api`、`/uploads`、`/openapi.yaml` 反代到此 origin（默认 `http://127.0.0.1:3000`；Compose 内为 `http://rest-api:3000`） |
...
**环境**：脚本合并 monorepo 根 dotenv 与 [`synthetic-it.env`](apps/backend/rest-api/scripts/synthetic-it.env)（`REST_API_*`、`SYNTHETIC_*` 等）；**`ADMIN_BOOTSTRAP_*` 仅来自根目录 `.env.*`**。管理员认证优先级：`REST_API_IMPORT_TOKEN` → `REST_API_IMPORT_USERNAME`/`PASSWORD` → `ADMIN_BOOTSTRAP_*` 登录；**用账号口令时，访问令牌（默认 15 分钟）过期会自动重新登录续期**，直接给 `REST_API_IMPORT_TOKEN` 则无法续期。详见 `synthetic-it-resolve-import-token.ts` 与 [`docs/admin-bootstrap.md`](docs/admin-bootstrap.md)。
...
| `packages/js-bridge`      | `@express-vue3-monorepo/js-bridge`    | 多宿主 H5 Bridge（浏览器 / 小程序 / App WebView）；**前端 app 未引用** |
| `packages/web-monitor`    | `@express-vue3-monorepo/web-monitor`  | Web Vitals 与客户端错误上报；**前端 app 未引用**                       |
