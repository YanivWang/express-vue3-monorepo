...
| `REFRESH_ROTATION_GRACE_SECONDS`                         | 可选；刷新令牌轮换后的并发宽限秒数，默认 **30**。多标签页/门户与管理端共用同一枚刷新 Cookie，窗口内的重复提交按竞态补发而非重放；设 **`0`** 为严格模式                                                                                                                                                                                           |
| `RATE_LIMIT_GLOBAL_MAX` 等 `RATE_LIMIT_*`                | 可选；限流阈值，默认全局 15 分钟 / 1000 次、注册登录 1 分钟 / 10 次、刷新接口 1 分钟 / 30 次（只统计失败请求）。**计数存于 Redis（key 前缀 `rl:`），多副本共用同一个桶**；Redis 异常时放行并记 error 日志，不因限流把请求打成 500                                                                                                                |
