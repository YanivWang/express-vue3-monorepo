...
function loadItCategorySeed(): ItCategorySeedFile {
  ...
  try {
    ...
    throw new Error(`[it-seed-categories] 无法读取 ${seedPath}: ${String(err)}`);
  }
  if (
    !parsed ||
    ...
    throw new Error(`[it-seed-categories] 无效的 JSON: ${seedPath}`);
  }
  ...
  for (const c of children) {
    if (!c || typeof c.name !== "string" || typeof c.sortOrder !== "number") {
      throw new Error(`[it-seed-categories] children 项格式错误: ${seedPath}`);
    }
  }
...
if (existing > 0) {
  console.log(`[it-seed-categories] 已有 ${existing} 条分类，跳过`);
...
console.log(
  `[it-seed-categories] 已写入根「${seed.rootName}」及 ${seed.children.length} 个叶子分类`,
);
