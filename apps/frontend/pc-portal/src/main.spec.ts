 ...
 * 守卫读到的恒为「未登录」，表现就是「刷新或直接输 URL 都被踢回登录页」。
 *
 * 这条顺序一旦被挪回去，typecheck、lint、组件测试与生产构建全都不会报错——
 * 它只会以「用户一刷新就掉登录」的形式暴露，正是它当初长期没被发现的原因。
 * 因此这里直接对启动过程本身断言。
 */
...
describe("pc-portal 启动顺序", () => {
  it("会话恢复完成之前不安装路由，避免守卫读到尚未结算的登录态", async () => {
