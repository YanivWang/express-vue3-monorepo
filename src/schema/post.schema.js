...
const listPostsQuerySchema = z
  .object({
    ...
    parentId: z.preprocess(
      ...
      z.string().trim().regex(/^[1-9]\d*$/, "parentId 必须是正整数").transform(Number).optional(),
    ),
    categoryId: z.preprocess(
      (v) => (v === "" || v === undefined || v === null ? undefined : v),
      z.string().trim().regex(/^[1-9]\d*$/, "categoryId 必须是正整数").transform(Number).optional(),
    ),
  })
...
const createPostBodySchema = postBodySchema.extend({
  categoryId: z.coerce.number({ error: "分类 ID 须为正整数" }).int("分类 ID 须为正整数").positive("分类 ID 须为正整数"),
});
...
export const updatePostSchema = z.object({
  ...
  body: postBodySchema.partial().extend({
    categoryId: z.coerce.number({ error: "分类 ID 须为正整数" }).int("分类 ID 须为正整数").positive("分类 ID 须为正整数").optional(),
  }),
});
