...
| `admin.categories.write`     | `POST /api/categories`（公开路由上的建叶子）；`POST /api/admin/categories/roots`、`POST /api/admin/categories/leaves`、`PATCH /api/admin/categories/:id` | -                                                                                                                              |
...
| `admin.roles.manage`         | `GET/POST/PATCH/DELETE /api/admin/roles`、`GET /api/admin/permissions`                                                                                   | `adminRole.service`；`PATCH` 带 `permissionCodes` 时，内置 `user` / `super_admin` 被 `assertPermissionsAssignable` 拒绝（400） |
